...
<div class="panel-card mb-4">
    <div class="panel-head">
        <div>
            <h5>System-wide appointments</h5>
            <p>Track appointment status, issued prescriptions, and the history of completed visits.</p>
        </div>
        <a href="{{ route('admin.medications.index') }}" class="btn btn-primary">Manage medication catalog</a>
    </div>

    <form method="GET" class="row g-3 filter-bar mt-3">
        <div class="col-md-4"><input type="text" name="keyword" value="{{ request('keyword') }}" class="form-control" placeholder="Appointment code, patient, doctor..."></div>
        <div class="col-md-3">
        ...
        <div class="col-md-3">
            <select name="status" class="form-select">
                ...
                @foreach(['pending' => 'Pending confirmation', 'confirmed' => 'Confirmed', 'completed' => 'Completed', 'cancelled' => 'Cancelled'] as $value => $label)
            ...
            <tbody>
                @forelse($appointments as $appointment)
                    <tr>
                        ...
                        <td>{{ \Illuminate\Support\Str::limit($appointment->diagnosis ?: 'None yet', 60) }}</td>
                        <td>{{ $appointment->prescriptions->count() }}</td>
                        <td>
                            @if($appointment->review)
                                <div class="fw-semibold text-warning">{{ str_repeat('★', (int) $appointment->review->rating) }}</div>
                                <div class="text-muted small">{{ $appointment->review->rating }}/5 · {{ $appointment->review->patient->full_name ?? 'Patient' }}</div>
                            @else
                                <span class="text-muted small">None yet</span>
                            @endif
                        </td>
                        <td class="text-end">
                            <a href="{{ route('admin.appointments.show', $appointment) }}" class="btn btn-sm btn-outline-primary">View details</a>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="9" class="text-center text-muted py-4">No matching appointments found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
